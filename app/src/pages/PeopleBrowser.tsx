...
// A person's whole photo list is fetched up front (unlike Photos/Folders'
// server-paginated time/folder buckets), but still rendered windowed like
// they are - without this, a person with thousands of photos mounted every
// single AssetTile's DOM subtree at once, which could peg the webview's
// render thread for seconds (invisible to BrightTable's own CPU/RAM readout,
// since that only samples the Rust process - see `get_resource_usage` - not
// the separate webview process actually doing the work) and, because it's a
// single-page app, made every tab feel frozen along with it. There's no
// real "bucket" to virtualize over here the way Folders has real folders,
// so this just chunks the flat asset list into fixed-size groups purely for
// windowing purposes.
const PEOPLE_GRID_CHUNK_SIZE = 60;
...
const PeopleBrowser = forwardRef<PeopleBrowserHandle, {
  ...
  // File Manager, and batch Rotate - shared with Photos/Folders/Albums via
  // useAssetActions.ts (see its own doc comment for why assetByIdAll/
  // commitEdit/commitEditMany are passed at call time below rather than to
  // the hook itself - the ordering matters because overlaidAssets right below
  // needs unsyncedMetadata/processingSidecarAssets before assetByIdAll can
  // exist). Headless Roundtrip/Tweak Roundtrip/Open in Ext. Editor stay
  // Photos/Folders-only (RAW-culling-pipeline concepts orthogonal to this
  // view, same reasoning as the Stack/ART omissions this view already had).
  const {
    unsyncedMetadata,
  ...
  useEffect(() => {
    ...
    getPerson(openPersonId)
      .then((p) => {
        ...
        // path mapping is configured (Preferences → Library → Originals on
        // Disk) or these assets don't resolve to one.
        scanUnsyncedMetadata(p.assets);
      })
      .catch((e) => setDetailError(String(e)));
  }, [openPersonId, scanUnsyncedMetadata]);
  ...
  const overlaidAssets = useMemo(
    ...
    [person, stackByAssetId, unsyncedMetadata, processingSidecarAssets],
  );
  ...
  const gridVirtualizer = useVirtualizer({
    ...
    // bucket virtualizer.
    estimateSize: () => Math.ceil(PEOPLE_GRID_CHUNK_SIZE / 6) * 124,
    overscan: 2,
  });

  useEffect(() => {
    // A newly-opened person's chunk heights have nothing to do with the
  ...
  // SelectionBar - until this person is closed and reopened.
  useEffect(() => {
...
function PersonCard({
  ...
  return (
    ...
    >
      <div style={{ aspectRatio: '1 / 1', borderRadius: '50%', overflow: 'hidden', position: 'relative', background: 'var(--surface-sunken)', boxShadow: '0 0 0 1px var(--border)' }}>
      ...
      <div style={{ fontSize: 11.5, color: 'var(--text-dimmer)', textAlign: 'center' }}>
